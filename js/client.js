...
var button_options = "buttons-only:#Go foward%%f#Turn left%%l#Turn right%%r";

var survey = [];

// --- DOM Elements ---
// const mapOutputDiv = document.getElementById('map-output');
...
var navigation_messages = []; // Store messages for chatbot mode

// --- Helper for appending messages to game output ---
function appendMessage(message, className = '') {
    if ( CHATBOT_MODE ) {
        message = message.replace(/\r?\n|\r/g, '<br>'); // Replace newlines with <br> for HTML display
...
var chatbot = new Chatbot(taketurn = function(chatbot, message) {
    ...
    if ( message == "OK, I understand." ) {
        display_status(); // Initial status display
        appendMessage(button_options);
        chatbot.talk(survey_next_question());
        return;
    }

    if ( message.includes("%%") ) message = message.split("%%")[1]; // remove comment
    answers.push(message); // Store the user's answer
    if (['f', 'l', 'r'].includes(message)) {
        move_player(message.toLowerCase());
        display_status();
        // Always check win condition after a move
        if (is_near_target()) {
            chatbot.talk(navigation_messages.concat([`Congratulations! You found the ${actual_target_building_name} near your current position!`]));
            task_completed = true;
        ...
        chatbot.talk(["Sorry, invalid command.",button_options]);
    }
...
var survey_next_question = function() {
    survey.push({"messages": navigation_messages});
    navigation_messages = []; // Reset for next round
...
var submit = function() {
    var res = {
        ...
        town_grid: town_grid,  // town_grid is a global variable defined in town.js
        player_trace: player_trace  // player_trace is a global variable defined in town.js
    };
    console.log(res);
